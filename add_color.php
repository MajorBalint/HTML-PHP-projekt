...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO Color (name, metallic, price) VALUES ('$name', '$metallic', '$price')";

    if (mysqli_query($conn, $sql)) {
        echo "A szín sikeresen hozzáadva.";
        mysqli_close($conn);
        header("Location: admin.php");
        exit;
    } else {
        echo "Hiba történt a szín hozzáadásakor: " . mysqli_error($conn);
        echo '<br><a href="admin.php">Vissza</a>';
    }
}
...
<div class="container">
    ...
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        <div class="form-group">
            <input type="text" class="form-control" id="name" name="name" placeholder="Szín" required>
        </div>
        <div class="form-group">
            <select class="form-control" id="metallic" name="metallic" required>
                <option value="0">Nem metál</option>
                <option value="1">Metál</option>
            </select>
        </div>
        <div class="form-group">
